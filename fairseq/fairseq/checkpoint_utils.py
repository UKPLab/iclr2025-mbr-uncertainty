...
from fairseq.optim.ivon import IVON
...
def load_model_ensemble_and_task(
    ...
    for filename in filenames:
            ...
            if "fsdp_metadata" in state and num_shards > 1:
                ...
                if sample_params:
                    optimizer = IVON(
                        ...
                        state["last_optimizer_state"]["param_groups"][0]["lr"],
                        state["last_optimizer_state"]["param_groups"][0]["ess"],
                        hess_init=1.0,
                        beta1=0.9,
                        beta2=0.99999,
                        weight_decay=state["last_optimizer_state"]["param_groups"][0]["weight_decay"],
                    )
                    for group_idx in range(len(state["last_optimizer_state"]["param_groups"])):
                        optimizer.param_groups[group_idx]["hess"] = state["last_optimizer_state"]["param_groups"][group_idx]["hess"]
                        if ess is not None:
                            optimizer.param_groups[group_idx]["ess"] = ess
        ...
        if sample_params:
            logger.warning(f"Sampling parameters for {num_mc_samples} MC samples from IVON.")

            if manual_seed is not None:
                torch.manual_seed(manual_seed)

            for i in range(num_mc_samples):
                with optimizer.sampled_params(train=False, restore=num_mc_samples > 1):
