...
class FairseqTask(object):
    ...
    def train_step(
        ...
        if hasattr(optimizer._optimizer, "sampled_params"):
            accumulated_loss = torch.zeros([]).to("cuda:0")
            for _ in range(optimizer._optimizer.mc_samples):
                with optimizer._optimizer.sampled_params(train=True):
                    model.set_num_updates(update_num)
            ...
            return accumulated_loss / optimizer._optimizer.mc_samples, sample_size, logging_output
        else:
            with torch.autograd.profiler.record_function("forward"):
                with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                    loss, sample_size, logging_output = criterion(model, sample)
